...
$j('.nav-offcanvas ul.level0, .nav-utility > ul').addClass('ul-basic');
...
$j('.nav-offcanvas li.ulMenuItem, .sub-menu ul > li').each(function() {
  ...
  $j(this).contents().filter(function() {
    return this.nodeType === 3 && this.nodeValue.trim().length != 0; // Node.TEXT_NODE
...
function configureGallery(gallerySettings) {
    ...
    gallerySettings.DelayImageDisplay = function () {
        ...
        $j("#lightBox" + gallerySettings.PluginID + "Div").removeAttr('style'); //jn: clears inline css
    ...
    gallerySettings.CloseLightBox = function () {
        ...
        disableDiv("lightBox" + gallerySettings.PluginID + "Div",0,0); //jn: runs disableDiv and resets width/height
        $j('body').scrollTo('[id*="photoPluginWrapper"]'); //jn: fixes ipad issue
        gallerySettings.LightboxIndex = -1;
    };
